...
- 修改点：更新 `raicom2026_x2_autonomy/src/preset_motion_wrapper.cpp/.h`，将默认 `zone_1` 目标调整为 `(-0.05, 1.60)`；提高前进、侧移、偏航控制上限；新增到达 `zone_1` 后顺时针原地旋转 `150` 度的二阶段控制；修正到点后旋转阶段被提前停掉的问题。
...
- 如何验证：在容器内重启 `run_all.sh`，确认日志包含 `STEP1_RESULT stable_stand=true`、`STEP3_RESULT filtered_navigation=true`、`NAV_GOAL zone_1 reached ... rotate_target_yaw=...` 和 `NAV_GOAL post-zone rotation complete ...`，并在仿真中观察机器人行走到 `zone_1=(-0.05,1.60)` 后顺时针继续旋转。
